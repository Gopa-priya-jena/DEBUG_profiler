...
std::string NOColor_format::Time_format(const TIME time) {
  ...
  auto time_fmt =
      std::format("[ELASPSED TIME]:{}hrs {}min {}s {}ms {}us {}ns", time.hour,
                  time.min, time.sec, time.milli, time.micro, time.nano);
  return time_fmt;
}
std::string NOColor_format ::Location_format(const Source_Loc loc) {

  auto Location =
      std::format("\n [FILE LOCATION]:\n {}{}{}", div, loc.file_name, div);

  auto Function = std::format("\n [FUNCTON]:{}{}\n", loc.function_name, div);

  auto line = std::format("\n [LINE NUMBER ]:{}{}", loc.line, div);
...
std::string NOColor_format::level_fmt(const log_level level) {
  ...
  return std::format("{} {} ->", Level_info[lvl].symbol, Level_info[lvl].name);
}
...
void NOColor_format::BIN_dump(const std::uint8_t *ptr, const std::uint64_t size,
                              Stream *stream)
...
void NOColor_format::HEX_dump(const std::uint8_t *ptr, const std::uint64_t size,
                              Stream *stream) {
