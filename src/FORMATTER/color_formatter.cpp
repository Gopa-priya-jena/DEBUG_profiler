...
std::string Color_format::Time_format(const TIME time) {
  ...
  auto time_fmt = std::format(
      "\n{}{}{}{} [ELASPSED TIME]:{}{}", RESET_SEQ, color_rgb_fg.format(purple),
      color_rgb_bg.format(DEFAULT_BG), _stopwatch,
      italic_mode.format(std::format("{}hrs {}min {}s {}ms {}us {}ns",

                                     time.hour, time.min, time.sec, time.milli,
                                     time.micro, time.nano)),
      RESET_SEQ);
  return time_fmt;
}
std::string Color_format ::Location_format(const Source_Loc loc) {
  ...
  auto Location =
      std::format("\n{}{}{}[FILE LOCATION]:{}", RESET_SEQ,
                  color_rgb_fg.format(plum), color_rgb_bg.format(DEFAULT_BG),
                  bold_mode.format(loc.file_name), RESET_SEQ);

  auto Function =
      std::format("\n{}{}{}[FUNCTON]:{}{}\n", RESET_SEQ,
                  color_rgb_fg.format(gold), color_rgb_bg.format(DEFAULT_BG),
                  italic_mode.format(loc.function_name), RESET_SEQ);

  auto line = std::format(
      "{}{}{}[LINE NUMBER ]:{}{}", RESET_SEQ, color_rgb_fg.format(royal_blue),
      color_rgb_bg.format(DEFAULT_BG),
...
Color_format::F_arg_format(const std::vector<std::string_view> &Args_name,
  ...
  OSS << "\n"
      ...
      << color_rgb_bg.format(DEFAULT_BG) << bold_mode.format("[FUNCTION ARGS]")
      << RESET_SEQ << "\n";

  for (auto i = 0; i < Args_name.size(); i++) {
    OSS << color_rgb_fg.format(*RGB_clrs_[RGB_start_ + i])
        << color_rgb_bg.format(DEFAULT_BG) << italic_mode.format(Args_name[i])
        << RESET_SEQ << "\n";

    OSS << color_rgb_fg.format(*RGB_clrs_[RGB_start_ + i + 2])
        << color_rgb_bg.format(DEFAULT_BG) << italic_mode.format(Arg_data[i])
        << RESET_SEQ << "\n";
  }
...
std::string Color_format::current_time_fmt(
  ...
  return std::format("\n{}{}{}{} [TIME]:{:%A %d %B %y -> %I:%M:%S %p }{}",
                     ...
                     color_rgb_bg.format(DEFAULT_BG), _clock, time, RESET_SEQ);
}
std::string Color_format::level_fmt(const log_level level) {
  ...
  return std::format(
      ...
      bold_mode.format(
          std::format(" {} {}", Level_info[lvl].symbol, Level_info[lvl].name)),
      _arrow, RESET_SEQ);
}
...
std::string Color_format::log_fmt(
  ...
  return std::format("{}{}{}{}{}", level_fmt(lvl), Time_format(e_time),
                     current_time_fmt(time), Location_format(loc), data);
}
...
std::string Color_format::trace_fmt(
  ...
  return std::format("{}{}{}{}{}", level_fmt(log_level::TRACE),
                     ...
                     Location_format(loc), F_arg_format(Args_name, Arg_data));
}
std::string Color_format::trace_fmt(
    const Source_Loc loc, const TIME e_time,
    const std::chrono::zoned_time<std::chrono::system_clock::duration> time) {
...
std::string Color_format::stack_trace_fmt(
  ...
  return std::format("{}{}{}", Time_format(e_time), current_time_fmt(time),
                     trace_fmt(str));
}

void Color_format::BIN_dump(const std::uint8_t *ptr, const std::uint64_t size,
                            Stream *stream) {
  std::ostringstream OSS;
  ...
  while (offset <= size) {
    ...
    std::bitset<4> L{VAL.to_ulong() & 0xF}, H{(VAL.to_ulong() >> 4) & 0xF};
...
void Color_format::HEX_dump(const std::uint8_t *ptr, const std::uint64_t size,
                            Stream *stream) {
...
void Color_format::OCTAL_dump(const std::uint8_t *ptr, const std::uint64_t size,
                              Stream *stream) {
...
void Color_format::mem_view(
  ...
  stream->StreamI(std::format("{}{}{}{}", color_rgb_fg.format(Yellow_green),
                              color_rgb_bg.format(DEFAULT_BG), var_name,
                              RESET_SEQ));
