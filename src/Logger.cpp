...
namespace DEBUG_profiler
{
  ...
  void Logger::set_mem_view( std::uint8_t view )
  {
    _memory_view = view;
  }
  std::vector<std::string_view>
  Logger::parse_argname( std::string_view arg_name )
  {
  ...
  TIME Logger::get_time_elaspsed()
  {
    ...
    Time.sec =
        ( std::chrono::floor<std::chrono::seconds>( fraction ) % 60s ).count();

    Time.milli =
        ( std::chrono::floor<std::chrono::milliseconds>( fraction ) % 60ms )
            .count();
    Time.micro =
        ( std::chrono::floor<std::chrono::microseconds>( fraction ) % 60us )
            .count();

    Time.nano =
        ( std::chrono::floor<std::chrono::nanoseconds>( fraction ) % 60ns )
            .count();
  ...
  Logger::Logger()
  {
    Profiler_conection_waiting // for starting the profiler
  ...
  void Logger::MEMORY_VIEW( const std::string_view file,
