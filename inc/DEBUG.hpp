...
inline void mem_info(std::string_view file, std::uint32_t line,
  ...
  LOGGER_.MEMORY_VIEW(file, line, function, argname, (std::uint8_t *)(&arg),
                      sizeof(Args), view);
}
// tracer
#if TRACE_ENABLE
...
#define FUNCTION_PROFILE_ARG(...)                                              \
  LOGGER_.trace(__FILE__, __LINE__, FUNCTION_NAME, #__VA_ARGS__, __VA_ARGS__); \
  // F_PROFILE
#endif
// logging
...
#define LOG_INFO(...)                                                          \
  LOGGER_.log(DEBUG_profiler::log_level::INFO, MSG_concat(__VA_ARGS__));
#endif
#if LOG_LEVEL_DEBUG_ENABLE
#define LOG_DEBUG(...)                                                         \
  LOGGER_.log(DEBUG_profiler::log_level::DEBUGGING, MSG_concat(__VA_ARGS__));
#if LOG_LEVEL_WARN_ENABLE
#define LOG_WARN(...)                                                          \
  LOGGER_.log(__FILE__, __LINE__, FUNCTION_NAME,                               \
              DEBUG_profiler::log_level::WARN, MSG_concat(__VA_ARGS__));
#endif
#endif
#if LOG_LEVEL_ERROR_ENABLE
#define LOG_ERROR(...)                                                         \
  LOGGER_.log(__FILE__, __LINE__, FUNCTION_NAME,                               \
              DEBUG_profiler::log_level::ERROR_, MSG_concat(__VA_ARGS__));
#endif
#define LOG_CRITICAL(...)                                                      \
  LOGGER_.log(__FILE__, __LINE__, FUNCTION_NAME,                               \
              DEBUG_profiler::log_level::CRITICAL, MSG_concat(__VA_ARGS__));
#endif
#endif
