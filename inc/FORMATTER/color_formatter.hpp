...
namespace DEBUG_profiler
{
  ...
  class Color_format : public Formatter
  {
    ...
    void        DECIMAL_dump( const std::uint8_t *ptr, const std::uint64_t size,
                              Stream *stream );
    std::string data_fmt( const log_level level, std::string_view data );
  ...
  public:
    ...
    void mem_view(
        const TIME e_time,
        const std::chrono::zoned_time<std::chrono::system_clock::duration> time,
        const Source_Loc Loc, std::uint8_t view,
        const std::string_view var_name, const std::uint8_t *ptr,
